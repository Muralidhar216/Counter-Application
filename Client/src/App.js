...
const Home = () => {
  ...
  const fetchCounters = useCallback(async () => {
    try {
      const [countResponse, myCountResponse] = await Promise.all([
        axios.get('http://localhost:5000/api/counter'),
        axios.get('http://localhost:5000/api/mycounter')
      ]);

      dispatch({ type: 'SET', count: countResponse.data.count });
      dispatch({ type: 'MYSET', mycount: myCountResponse.data.mycount });
    } catch (err) {
      console.error(err);
    }
  }, [dispatch]);
